...
<div class="row-fluid span12">
				 <div class="span12">
					 ...
					 <?php
						  ...
						  if($campo!==null)
						  {
							  $step.='<li style="background: #8DC641;" class="active"><span class="badge"></span>'.CHtml::link("Campo",array('vcampo', 'id'=>$model->id)).'<span class="chevron"></span></li>';
						  }
	...
	{     //Ultima Inspeccion
		switch ($inspection->inspection_number)
		{
		  ...
		  if($inspection->inspection_number==$max->inspection_number)
		  {
					foreach($inboxs as $inbox)
					{
								...
								if($inbox->status_id==4 && $inspection->proposed_time==null && $acond==null )//Acondicionamientos
								{
										  ...
										  <div class="span12">
											...
											<?php if( $cantidadpayment->form_id>0){?>
											
											<?php $this->widget('bootstrap.widgets.TbButton', array(
																						'id'=>'btn_acon',
																					   'type'=>'primary',
																					   'label'=>'Solicitar '.$texto.' Inspección',
																					   'buttonType'=>'ajaxLink',
																					   'disabled'=>'disabled',
																						'htmlOptions'=>array(),
																					
																					  ));?>
										  <p style="color: #b94a48">Usted tiene un pago pendiente debe generar su nota de pago</p>
											<?php }else{ ?>
											...
											<!--Solicitar Inspeccion-->
												<div id="myModal_inspeccion" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
																	  <div class="modal-header">
																		 ...
																		 <h4 id="myModalLabel">Solicitar Inspección de Campo Semillero</h4>
																	  </div>
					...
					foreach($inboxs as $inbox)
					{
					 if($inbox->status_id==7 && $inspection->proposed_time!==null && $headquarter->tipo_empresa==2) //&& $inspection->real_date==null && $inspection->proposed_time!==null)
					 {?>
						 <div class="row-fluid">
							 <div class="span2">
					...
					{			
						...
						{?>
									...
									<div class="row-fluid">
										...
										<div class="span10">
											<?php echo CHtml::link("<b>Informe de $texto Inspección</b> </br>",array('pdf/Semilla','id'=>$model->id,'idinspect'=>$idinspect,'identificador'=>1));?>
										</div>
									</div>											
					...
					{?>
						<div class="row-fluid">
							<div class="span2"></div>
							<div class="span10">
								<?php echo CHtml::link("<b>Informe de  $texto Inspección Rechazado</b>",array('pdf/Semilla','id'=>$model->id,'idinspect'=>$idinspect,'identificador'=>3)); ?>
							</div>
						</div>
						...
						<div class="row-fluid">
							...
							<div class="span10">
								<div class="row-fluid">	
									 ...
									 <div class="row-fluid">
										  <div class="span12">
											<?php echo "<b>Solicitud de $texto Inspección: </b><br>".date("d-m-Y",strtotime($inspection->proposed_date))."&nbsp &nbsp".date("h:m A",strtotime($inspection->proposed_time)); ?>
										  </div>
									 </div>
									 <div class="row-fluid">
		  ...
		  { ?>
				<?php foreach($inboxs as $inbox)
					{
					 if($inbox->status_id==7 && $headquarter->tipo_empresa==2) //&& $inspection->real_date==null && $inspection->proposed_time!==null)
					 {?>
						 <div class="row-fluid">
							 <div class="span2">
								<?php echo date("d-m-Y",strtotime($inbox->date));?>
